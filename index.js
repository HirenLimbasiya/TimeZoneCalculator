...
// ------------------------------------conunty with more than one zone------------------------
...
//---------------------------------- uniqe country with no repeatation---------------------------------------
...
const fetchData = async (input1) => {
  const data = await fetch("https://api.timezonedb.com/v2.1/list-time-zone?key=38XV6NBA5L1F&format=json");
...
const withoutChecked = async (ans1, ans2 , time) => {
  ...
  let f = `https://api.timezonedb.com/v2.1/convert-time-zone?key=38XV6NBA5L1F&format=json&from=${ans1}&to=${ans2}&time=${time}`;
  let a = await fetch(f)
  ...
  if(checked.checked){
    ...
    if(data.offset === 0){
        ...
        result.innerHTML = `${timeZone2[0].countryName} is <span class="key">${dis ? "AHEAD" : "BACK"}</span> with <span class="key">${timeDiff} HOURS</span> from ${timeZone1[0].countryName}`
...
function getData() {
  ...
  if (checked.checked) {
    ...
    const unixTimestamp =Number(Math.floor(timestampInMs / 1000))  + 19800 // convert indian to glob ;
...
// ------------------------------------rendering for morethan one zone---------------------------
